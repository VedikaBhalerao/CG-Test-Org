    ...
    rewardProducts.forEach(function(item) {
      ...
      if(itemsInOrder.length > 0 ) {
        ...
        if(itemInOrder.getQuantity() !== item.getQuantity()) {
          ...
          if(!isPerFactorChanged && ((item.getDeliveryState() === "Available" && itemTemplate.getConsiderDeliveryState() == "1")  || itemTemplate.getConsiderDeliveryState() == "0")) {
            itemInOrder.setQuantity(item.getQuantity());
          } else {
            item.setQuantity(oldValue);
          }
